...
import { Sparkles, Download, Calendar, RotateCcw, Copy, Check, Edit3, Save, Loader2, ArrowLeft, Hash, FileText, Type, MessageSquare } from 'lucide-react';
...
export default function ReelPreviewPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/reels/${reelId}`)
      .then(r => r.json())
      .then(d => { setReel(d); setEditCaption(d?.caption ?? ''); setEditDescription(d?.description ?? ''); setEditHashtags((d?.hashtags ?? []).join(' ')); })
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/reels/${reelId}`, {
        ...
        body: JSON.stringify({ caption: editCaption, description: editDescription, hashtags: editHashtags.split(/\s+/).filter(Boolean) }),
      });
  ...
  if (!reel) return <div className="text-center py-20 text-white/40">Reel not found</div>;
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => copyToClipboard(editCaption)} className="p-1.5 rounded hover:bg-white/5"><Copy className="w-3.5 h-3.5 text-white/40" /></button>
                    <button onClick={() => setEditing(!editing)} className="p-1.5 rounded hover:bg-white/5"><Edit3 className="w-3.5 h-3.5 text-white/40" /></button>
                  </div>
                </div>
                {editing ? (
                  <div className="space-y-3">
                    ...
                    <button onClick={handleSave} disabled={saving} className="px-4 py-2 rounded-lg gold-gradient text-black text-xs font-semibold flex items-center gap-1.5">
                      {saving ? <Loader2 className="w-3 h-3 animate-spin" /> : <Save className="w-3 h-3" />} Save
                    </button>
                  </div>
                ) : (
                  <p className="text-sm text-white/70 leading-relaxed">{reel?.caption ?? 'No caption'}</p>
                )}
              </div>
            )}
            ...
            {activeTab === 'hashtags' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => copyToClipboard((reel?.hashtags ?? []).join(' '))} className="p-1.5 rounded hover:bg-white/5"><Copy className="w-3.5 h-3.5 text-white/40" /></button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {(reel?.hashtags ?? []).map((tag: string) => (
                    <span key={tag} className="px-3 py-1 rounded-full bg-[#7B2FBE]/10 text-[#A855F7] text-xs">{tag}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {activeTab === 'description' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => copyToClipboard(editDescription)} className="p-1.5 rounded hover:bg-white/5"><Copy className="w-3.5 h-3.5 text-white/40" /></button>
                </div>
                {editing ? (
                  ...
                  <p className="text-sm text-white/70 leading-relaxed">{reel?.description ?? 'No description'}</p>
                )}
              </div>
            )}
          </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {Object.entries(costBreakdown ?? {}).map(([key, val]: [string, any]) => (
                  <div key={key} className="text-center p-2 rounded-lg bg-white/[0.02]">
                    <p className="text-[10px] text-white/30 uppercase">{key.replace('_', ' ')}</p>
