...
async function generateOnce(prompt: string, apiKey: string, styleSuffix: string): Promise<string> {
 ...
 */
async function generateOne(prompt: string, apiKey: string, styleSuffix: string): Promise<string> {
  let lastErr: any;
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      return await generateOnce(prompt, apiKey, styleSuffix);
    } catch (e) {
      lastErr = e;
...
export class AbacusImageProvider implements Provider<ImageInput, ImageOutput> {
  ...
  async generate(input: ImageInput): Promise<ImageOutput> {
    ...
    // Resolve mood-aware style suffix so images match the user's intention.
    const styleSuffix = getMoodStyle(input?.mood).imageSuffix;
    ...
    async function worker() {
      while (idx < scenes.length) {
        const i = idx++;
        results[i] = await generateOne(scenes[i].imagePrompt, apiKey!, styleSuffix);
      }
    }
